...
\item {\footnotesize ...am Anfang sind jedoch Mechanikprobleme zu empfehlen (meine Meinung)}
\end{itemize}
\end{frame}
...
\begin{frame}{Datenaufnahme und -auswertung}
  Datenerhebung hängt erheblich vom Problem ab. Zur Verfügung stehen jedoch ...
  \begin{itemize}
    \item Kraftsensor, Multimeter, Thermometer, Hallsonde, ...
    \item die im Schulunterricht verwendeten Materialien
    \item und weiteres ...
  \end{itemize}
  zur Verfügung.
  \vfill
  \pause
  Datenanalyse abhängig von Präferenz, Erfahrung und Daten:
  \begin{itemize}
...
\column{0.5\textwidth}
\begin{equation*}
\varphi = arctan\left(\frac{x_0 \omega_s}{\dot{x}_0}\right)
\end{equation*}
...
\begin{itemize}
  ...
  \item[$\rightarrow$] Physikvorwissen aus dem Physikgrundstudium kann vorausgesetzt werden 
\end{itemize}
\end{frame}

\begin{frame}{Diskussion (10 min.)}
Als Reporter ...
\begin{itemize}
\item meist passiv
...
\begin{frame}{Beispiel: \emph{Friction Oscillator}}
  ...
  \pause 
  \begin{itemize}
    \item[$\rightarrow$] Versucht die gesamten 10 Minuten auszuschöpfen
  \end{itemize}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
\node[align=center, inner sep=10pt, fill=green!30!white, draw, text width=8cm] (1) at (0, 0) {Heute, 13.09.2022};
\node[align=center, inner sep=10pt, fill=green!30!white, draw, text width=8cm] (2) at (0, -2) {Anmeldung der Teams, 01.12.2022};
\node[align=center, inner sep=10pt, fill=blue!30!white, draw, text width=8cm] (3) at (0, -4) {Regionalwettbewerb, ???};
\node[align=center, inner sep=10pt, fill=red!30!white, draw, text width=8cm] (4) at (0, -6) {Bundeswettbewerb, 03.03.2023};


\draw[->, thick] (1.south) -- (2.north) node[pos=0.5, right=3pt] {$\approx$ 10 Wochen};
\draw[->, thick] (2.south) -- (3.north) node[pos=0.5, right=3pt] {$\approx$ ? Wochen};
\draw[->, thick] (3.south) -- (4.north) node[pos=0.5, right=3pt] {$\approx$ ? Wochen};
